...

{
	...
	mMatch = new DuelMatch(mLeftInput.get(), mRightInput.get(), false, false, mRecorder.get());
	...
	/// \todo we need to get the gamespeed from somewhere else
	leftStream.Write((int)SpeedController::getMainInstance()->getSpeed());
	...
	/// \todo we need to get the gamespeed from somewhere else
	rightStream.Write((int)SpeedController::getMainInstance()->getSpeed());
