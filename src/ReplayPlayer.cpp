...
bool ReplayPlayer::play(DuelMatch* virtual_match)
{
	...
	if( mPosition < mLength )
	{
		...
		int point;
			std::cout <<mPosition << " " << loader->getSavePoint(mPosition, point).index << "\n";
		SavepointIndex index;
		if(loader->isSavePoint(mPosition, index))
		{
			std::cout << "SET REFERENCE PT";
			ReplaySavePoint reference;
			loader->readSavePoint(index, reference);
...
bool ReplayPlayer::gotoPlayingPosition(int rep_position, DuelMatch* virtual_match)
{
	...
	// find next safepoint
	int save_position = -1;
	SavepointIndex savepoint = loader->getSavePoint(rep_position, save_position);
	...
	if( (rep_position < mPosition || save_position > mPosition) && savepoint.isValid())
	{
		// can't use mPosition
