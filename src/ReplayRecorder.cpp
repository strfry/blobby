...
void ReplayRecorder::send(boost::shared_ptr<GenericOut> target) const
{
	...
	target->generic<std::vector<unsigned char, CountingAllocator<unsigned char, r_tag> > >(mSaveData);
...
void ReplayRecorder::receive(boost::shared_ptr<GenericIn> source)
{
	...
	source->generic<std::vector<unsigned char, CountingAllocator<unsigned char, r_tag> > >(mSaveData);
...
void ReplayRecorder::writeInputSection(boost::shared_ptr<GenericOut> file) const
{
	...
	file->generic<std::vector<unsigned char, CountingAllocator<unsigned char, r_tag> > > (mSaveData);
